...
export default function EventRecommendations() {
  ...
  const filteredEvents = recommendedEvents.filter((event) => {
    ...
    if (categories.includes(activeTab)) return event.category === activeTab
    return event.source === activeTab
  })

  // Get unique sources (excluding categories)
      ...
      <CardContent>
        <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="mb-4 flex flex-wrap">
            ...
            {sources
              .filter((source) => !categories.includes(source as string))
              .map((source) => (
                <TabsTrigger key={`source-${source}`} value={source}>
                  {source.split(".")[0]}
                </TabsTrigger>
              ))}
          </TabsList>
